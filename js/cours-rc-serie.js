/* ═══════════════════════════════════════════════
   MEGANE_LEARN — cours-rc-serie.js
   Diagramme de Fresnel pour le cours RC Série
...
function drawFresnel() {
  ...
  // Échelle auto
  const maxV = Math.max(UR, UC, U) * 1.2;
  ...
  // UC (en retard de 90° sur I, donc vers le bas sur le diagramme)
  const UCy = UC * scale;
  drawArrow(ox + URx, oy, ox + URx, oy + UCy, '#ffc94a', 2.5);
  ctx.fillStyle = '#ffc94a';
  ctx.fillText('UC', ox + URx + 12, oy + UCy/2);
