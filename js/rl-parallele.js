...
function calculateRLParallele() {
  ...
  const I = Math.sqrt(IR * IR + IL * IL);
  const Z = (I === 0) ? Infinity : U / I;
  const phi = Math.atan2(IL, IR);
  const phiDeg = phi * 180 / Math.PI;
  const cosphi = IR / I;
  const P = U * I * cosphi;
  const Q = U * I * Math.sin(phi);
  const S = U * I;
  return { U, R, L, f, omega, XL, IR, IL, I, Z, phi, phiDeg, cosphi, P, Q, S };
}

function updateCalculs(data) {
  const ids = ['omega', 'XL', 'IR', 'IL', 'I', 'Z', 'phi', 'cosphi', 'P', 'Q', 'S'];
  ids.forEach(id => {
...
function drawFresnel(data) {
  ...
  const maxI = Math.max(data.IR, data.IL, data.I) * 1.2;
...
function calculateAll() {
  const data = calculateRLParallele();
  updateCalculs(data);
