...
function calculateRC() {
  ...
  const Z = (XC === Infinity) ? Infinity : Math.sqrt(R * R + XC * XC);
  ...
  const sinphi = (Z === Infinity || Z === 0) ? 0 : -XC / Z;
  const P = U * I * cosphi;
  const Q = U * I * sinphi;
  const S = U * I;
  return { U, R, C, f, omega, XC, Z, I, UR, UC, phi, phiDeg, cosphi, sinphi, P, Q, S };
}

function updateCalculs(data) {
  const ids = ['omega', 'XC', 'Z', 'I', 'UR', 'UC', 'phi', 'cosphi', 'P', 'Q', 'S'];
  ids.forEach(id => {
...
function updateFormules(data) {
  ...
  if (document.getElementById('form_U')) document.getElementById('form_U').textContent = formatValue(Math.sqrt(data.UR*data.UR + data.UC*data.UC), 'V');
...
function drawSchema() {
  ...
  const cx = rx + rw + W*0.08, cw = W*0.25;
  ctx.beginPath();
  ctx.moveTo(cx, my - 15);
  ctx.lineTo(cx + cw/2, my - 25);
  ctx.lineTo(cx + cw/2, my - 5);
  ctx.fillStyle = 'transparent';
  ctx.stroke();
  ctx.fillStyle = '#4f7cff';
  ctx.fillText('C', cx + cw/2, my - 30);
  ctx.beginPath(); ctx.moveTo(cx + cw, my); ctx.lineTo(W - mx, my); ctx.stroke();
...
function drawFresnel(data) {
  ...
  const maxV = Math.max(data.UR, data.UC, data.U) * 1.2;
  ...
  const UCy = data.UC * scale;
  drawArrow(ox + URx, oy, ox + URx, oy - UCy, '#ffc94a', 2.5);
  ctx.fillStyle = '#ffc94a'; ctx.fillText('UC', ox + URx + 12, oy - UCy/2);
