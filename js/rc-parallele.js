...
function calculateRCParallele() {
  ...
  const I = Math.sqrt(IR * IR + IC * IC);
  const Z = (I === 0) ? Infinity : U / I;
  const phi = Math.atan2(-IC, IR);
  ...
  const Q = U * I * sinphi;
  const S = U * I;
  return { U, R, C, f, omega, XC, IR, IC, I, Z, phi, phiDeg, cosphi, sinphi, P, Q, S };
}

function updateCalculs(data) {
  const ids = ['omega', 'XC', 'IR', 'IC', 'I', 'Z', 'phi', 'cosphi', 'P', 'Q', 'S'];
  ids.forEach(id => {
...
function drawFresnel(data) {
  ...
  const maxI = Math.max(data.IR, data.IC, data.I) * 1.2;
  ...
  drawArrow(ox + IRx, oy, ox + IRx, oy - ICy, '#ffc94a', 2.5);
  ctx.fillStyle = '#ffc94a'; ctx.fillText('IC', ox + IRx + 12, oy - ICy/2);
...
function calculateAll() {
  const data = calculateRCParallele();
  updateCalculs(data);
